<script lang="ts">
  ...
  // The hero opens itself for a visitor who has just arrived: after a beat the
  // runway is scrolled for them, through the CTA frame (past CTAS_AT) so the
  // heart, the copy and the buttons are all in. The mechanism — when it
  // declines, what cancels it, the easing — is shared with the masthead that
  // opens /about and /donate; see $lib/utils/autoOpen.
  const AUTO_OPEN_THROUGH = 0.8;
